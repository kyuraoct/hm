...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    for (let i of json.result) {
        stiker = await sticker(false, i, `${pushname}`, `Sticker By Miko
Bot WhatsApp`)
...
*Estimasi selesai:* ${json.result.stickers.length * 1.5} detik`.trim())
    ...
    for (let i = 0; i < json.result.stickers.length; i++) {
        ...
        let stiker = await sticker(false, "https://api.telegram.org/file/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/" + jisin.result.file_path, `${pushname}`, `Sticker By Miko
Bot WhatsApp`)
